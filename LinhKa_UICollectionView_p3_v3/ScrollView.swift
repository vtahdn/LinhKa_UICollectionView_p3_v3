    ...
    override func viewDidLayoutSubviews() {
        ...
        if (!first) {
            ...
            for i in 0 ..< imageCount {
                ...
                let tap = UITapGestureRecognizer(target: self, action: #selector(ScrollView.zoomIn(_:)))
                tap.numberOfTapsRequired = 1
                imgView.addGestureRecognizer(tap)
                
                // Set zoom out by a double tap
                let doubleTap = UITapGestureRecognizer(target: self, action: #selector(ScrollView.zoomOut(_:)))
